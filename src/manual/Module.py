...
class Module:
    ...
    def load(self, module_dir_name: str, module_name: str):
        package_name = ".".join([self.worlds_dir_name, self.world_name, module_dir_name])
        ...
        try:
            if not self.world_source.is_zip:
            ...
            spec = importer.find_spec(f"{self.world_name}/{module_dir_name}")

            if spec is None:
                if self.debug:
                    print(f"WARNING: {self.world_name}.{module_dir_name} is None")
                    
                return None

            spec = importer.find_spec(f"{self.world_name}/{module_dir_name}/{module_name}")

            if spec is None:
                if self.debug:
                    print(f"{self.world_name}.{module_dir_name}.{module_name} is None")

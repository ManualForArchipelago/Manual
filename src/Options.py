...
def build_options(data: Namespace):
    ...
    for option_name, option in data.option_table.get('core', {}).items():
    ...
    for option_name, option in data.option_table.get('user', {}).items():
        if option_name.startswith('_'): #To allow commenting out options
            continue
        option_display_name =  option_name
        option_name = format_to_valid_identifier(option_name)
    ...
    for category in data.category_table:
        for option_name in data.category_table[category].get("yaml_option", []):
            if option_name[0] == "!":
    ...
    if data.starting_items:
        for data.starting_items in data.starting_items:
            if data.starting_items.get("yaml_option"):
                for option_name in data.starting_items["yaml_option"]:
                    if option_name[0] == "!":
                        option_name = option_name[1:]
                    option_name = format_to_valid_identifier(option_name)
                    if option_name not in manual_options:
    ...
    def make_options_group() -> list[OptionGroup]:  # TODO this is weird
